...
class HMAX(nn.Module):
    ...
    """
    def __init__(self, universal_patch_set, s2_act='gaussian', until_s1=False, until_c1=False, until_s2=False):
        super().__init__()
        ...
        self.until_s1 = until_s1
        self.until_c1 = until_c1
        self.until_s2 = until_s2
    ...
    def run_all_layers(self, img):
        ...
        if self.until_s2:
            return s1_outputs, c1_outputs, s2_outputs
    ...
    def forward(self, img):
        ...
        last_layer = self.run_all_layers(img)[-1]
        if self.until_s1:
            s1_outputs = last_layer
            return torch.cat([x.reshape(-1, 1) for x in s1_outputs])
        if self.until_c1:
            c1_outputs = last_layer
            return torch.cat([x.reshape(-1, 1) for x in c1_outputs])
        else:
            c2_outputs = last_layer
    ...
    def get_all_layers(self, img):
        ...
        """
        if self.until_s1:
            s1_out = self.run_all_layers(img)
            return (
