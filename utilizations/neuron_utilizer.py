...
#
# k-population:
#   for spiking model (syn, time) - > (stim_time):
#       (syn, time) -> (syn, time, k) [copy along z axis]
#       (syn, time, k) -> (syn, time, k) [masking]
#       (syn, time, k) -> (stim_time, k) [run through model k times]
#       (stim_time, k) -> (1) [decoding]
#
#   for non spiking model (*input_shape) -> (*output_shape)
#       (*input_shape) -> (*input_shape, k) [copy along z axis]
#       (*input_shape, k) -> (*input_shape, k) [masking]
#       (*input_shape, k) -> (*output_shape, k)  [run through model k times]
#       (*output_shape, k) -> (1) [decoding]
#
#   the generic option (*input_shape, time) -> (*output_shape, stim_time)
#   for the spiking neuron model input_shape = (syn), output_shape = (1)
#   for a general spiking model input_shape = (#input neurons), output_shape = (#output neurons)
#   for the non spiking model, stim_time=time=1
#       (*input_shape, time) -> (*input_shape, time, k) [copy along z axis]
#       (*input_shape, time, k) -> (*input_shape, time, k) [masking]
#       (*input_shape, time, k) -> (*output_shape, stim_time, k)  [run through model k times]
#       (*output_shape, stim_time, k) -> (1) [decoding]
#       
#

#   1) flatten
#       (*ds_input_shape, time) -> (ds_input_dim, time)
#   optional 2) wiring layer [mandatory if ds_input_dim != model_input_dim]
#       (ds_input_dim, time) -> (model_input_dim, time)
#   optional 3) masking layer
#           (model_input_dim, time) -> (model_input_dim * time, 1) [flatten]
#           (model_input_dim * time, 1) -> (model_input_dim * time, pred_time) [copy along y axis]
#           (model_input_dim * time, pred_time) -> (model_input_dim * time, pred_time) [masking]
#   4) model
#       variant a) 
#           run the model once [here pred_time := model_output_dim]
#              (model_input_dim, time) -> (pred_time)
#       variant b)
#           run the model pred_time times
#               (model_input_dim * time, pred_time) -> (pred_time)
#   5) decoding layer (max pooling / linear layer + non-linearity)
#       (pred_time) -> (1)
...
# wiring for any neuron nn / if / ff / bench lr / bench nn:
#   functional only [syn=axon]
#       1) (axon, total_time) * (axon, total_time) -> (axon, total_time)
#       2)  model ((axon, total_time)) -> (stim_time) [binary vector]
#   structural functional
#       1) (axon, total_time) x (syn, axon) -> (syn, total_time)
#       2)  model ((syn, total_time)) -> (stim_time) [binary vector]

# bench lr / bench nn would return a binary vector of length stim_time
# Think: what should be the threshold for lr? maybe 0.5?

# decoding:
#   trivial: max pooling
#   decoding layer: linear layer + non-linearity

# masking (aka time dependent wiring):
#   the idea is to highlight different parts of the input for the same model
#   For Idan, this is done for the lr / nn benchs
#   For Daphna, this is done to run the model multiple times
#   Think: might be also conv for Daphna? and also contain some non linearity?
#       (can be fixed by using external module)
...
# Think: the lr / dnn bench can go with 3) or without, why is that?
# Think: what about non classification problems? -> regression / generative?

# Think: maybe spiking and non spiking separately

# spiking model: (model_input_dim, time) -> (model_output_dim, pred_time)
#   where model_input_dim is the number of input neurons
#   where model_output_dim is the number of output neurons
#   for a single neuron / tcn / if / ff / spiking lr, model_output_dim=1

# non spiking model: (model_input_dim) -> (model_output_dim)

# wiring layer: means to reuse the same model for the a different task
# spiking model -> synaptic plasticity
# non spiking model -> shape change

# masking (reusing?) layer: to reuse/discover computations in the same model
# relevant mostly for non spiking models
# but it creates a "spiking-like" model, just because the "time" axis was introduced
#   similar to the edge case of a spiking model would generate independent outputs at each time point

# One can view any non spiking model as a spiking model with time=1,
# but is it useful?
# also the masking layer is only used for non spiking models, 
# it has no natural meaning for biophysical neuron
#   maybe distributing it to multiple different neurons?
#       ala a network of biophysical neurons
#   so maybe this is like an "self - ensemble" layer
#   also like creating population code
#       ala "population" layer
#   to use depth=2 biophysical network one should add a "model" layer

# perfect analogies:
#   neuron <-> model
#   wiring + neuron + decoding <-> wiring + model + decoding
#   wiring + masking + neurons + decoding  <-> population coding  <-> wiring + masking + models + decoding
#   wiring + masking + neurons + wiring + neuron + decoding <-> 2-network 
#       <-> wiring + masking + models + wiring + model + decoding
# can be generalized to an d-network

# bench:
# wiring + spiking lr + decoding
# spiking lr: (syn, total_time) -> (stim_time) [binary vector]

# Think: but there is still that time problem
#       is really the analogy perfect? 
#       to use time=1?
#       or to somehow consider the output of the "population" layer as a "spiking-like" model?

# maybe the spiking lr should get only the relevant information in every time-point?
#   so this is using some form of masking layer?
#       not quite, because using different weights/model?
#   this is <= regular spiking lr

# regular spiking lr: (syn, total_time) -> (syn * total_time) -> (stim_time) [binary vector]
#                     #params = syn * total_time * stim_time

# causal spiking lr: (syn, total_time) -> (syn * total_time, stim_time) [padded from left]
#                                      -> (stim_time) [binary vector]
#                   #params = syn * (pad_time + pad_time + 1 + ... + pad_time + stim_time)
#                           = syn * pad_time * (total_time - stim_time) + syn * pad_time * (1 + ... + stim_time)

# regular spiking lr using masking: (syn, total_time) -> (syn * total_time, stim_time)
#                                                     -> (stim_time) [binary vector]
#                                  #params = syn * total_time * stim_time
# exactly the same, but can be seen as a single model (spiking lr)
# or a population of models (population of lrs)
# the more natural way to compare against a spiking model, is to view it as an spiking lr
# for Idan, the way is spiking lr

# running the same model (for example resnet) stim_time times, for the project with Daphna,
# can also be seen in two ways,
# 1) as a population of such models (self ensemble)
# 2) as a "spiking-like" variant
# what is the more natural way to look at it?
# I think 1 is the way to go, because it is not really spiky (not binary or even prob)
...
# Think: rewrite the generic algo with "population" layer
...
# Think: also generic code for the "surrogate" idea (meaning also going back and checking it)